...
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=∆RY∆N-TECH&fontSize=72&fontColor=fff&animation=twinkling&fontAlignY=32&desc=High%20Performance%20WhatsApp%20Bot&descAlignY=55&descSize=20" width="100%"/>
...
[![Version](https://img.shields.io/badge/Version-6.0.0-blue?style=for-the-badge&logo=github)](https://github.com/GlobalTechInfo/∆RY∆N-TECH)
...
[![Stars](https://img.shields.io/github/stars/GlobalTechInfo/∆RY∆N-TECH?style=for-the-badge&logo=starship&color=gold)](https://github.com/GlobalTechInfo/∆RY∆N-TECH/stargazers)
[![Forks](https://img.shields.io/github/forks/GlobalTechInfo/∆RY∆N-TECH?style=for-the-badge&logo=git&color=orange)](https://github.com/GlobalTechInfo/∆RY∆N-TECH/network/members)
...
```bash
git clone https://github.com/GlobalTechInfo/∆RY∆N-TECH.git
cd ∆RY∆N-TECH
npm install
cp sample.env .env
...
```env
SESSION_ID=GlobalTechInfo/∆RY∆N-TECH_xxxxxxxxxxxxxxxxxxxxxxxx
```
...
```env
# ── REQUIRED (choose one) ────────────────────────────────────
SESSION_ID=GlobalTechInfo/∆RY∆N-TECH_your_gist_id_here
# OR
PAIRING_NUMBER=255637518095
...
```bash
sudo bash <(curl -fsSL https://raw.githubusercontent.com/GlobalTechInfo/∆RY∆N-TECH/main/lib/install.sh)
```
> [!IMPORTANT]
...
```bash
# After install:
nano /root/∆RY∆N-TECH/.env
cd /root/MEGA-MD && pm2 start dist/index.js --name Aryan 
...
tmux new -s ∆RY∆N-TECH    # Start new session
npm start
...
| `pm2 restart ∆RY∆N-TECH | Restart |
| `pm2 stop ∆RY∆N-TECH` | Stop |
...
2. Paste: `https://github.com/GlobalTechInfo/∆RY∆N-TECH
3. Open **Secrets** tab (🔒) and add:

   | Key | Value |
   |---|---|
   | `SESSION_ID` | `GlobalTechInfo/∆RY∆N-TECH_your_gist_id` |
   | `OWNER_NUMBER` | `255637518095 |
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/GlobalTechInfo/∆RY∆N-TECH/main/lib/heroku.sh)
```
**Manual:**
```bash
heroku login
...
heroku config:set SESSION_ID=GlobalTechInfo/∆RY∆N-TECH_your_gist_id
heroku config:set OWNER_NUMBER=255637518096
...
**One-line Deployer:**
```bash
bash <(curl -s https://raw.githubusercontent.com/GlobalTechInfo/∆RY∆N-TECH/main/lib/fly.sh)
```
**Manual:**
```bash
...
fly secrets set SESSION_ID=GlobalTechInfo/∆RY∆N-TECH_your_gist_id
fly secrets set OWNER_NUMBER=255637518095
fly deploy
...
# Run
docker run -d \
  -e SESSION_ID=GlobalTechInfo/∆RY∆N-TECH_your_gist_id \
  -e OWNER_NUMBER=255637518095 \
  -p 5000:5000 \
  --name mega-md \
  mega-md

# Logs
docker logs -f ∆RY∆N-TECH
```
...
# PostgreSQL
POSTGRES_URL=postgresql://user:password@host:5432/∆RY∆N-TECH

# MySQL
MYSQL_URL=mysql://user:password@host:3306/∆RY∆N-TECH
...
|---|---|---|---|
| `SESSION_ID` | ✅ *one of* | — | From aryan-pairing.onrender.com |
| `PAIRING_NUMBER` | ✅ *one of* | — | Phone number for terminal pairing |
| `OWNER_NUMBER` | ✅ | `255637518095` | Your number, no `+` |
| `BOT_NAME` | ❌ | `∆RY∆N-TECH` | Bot display name |
| `BOT_OWNER` | ❌ | `eddy | Owner display name |
| `PACKNAME` | ❌ | `∆RY∆N-TECH | Sticker pack name |
...
export default {
    ...
    async handler(sock: any, message: any, args: any[], context: any = {}) {
        const {
            ...
            channelInfo       // ∆RY∆N-TECH branding spread
        } = context;
...
> [!IMPORTANT]
> - Verify `SESSION_ID` starts with `GlobalTechInfo/∆RY∆N-TECH_`
...
[![GitHub Issues](https://img.shields.io/badge/GitHub_Issues-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/GlobalTechInfo/∆RY∆N-TECH/issues)
...
[MIT License](LICENSE) · Made with ❤️ by **∆RY∆N-TECH** · [GlobalTechInfo](https://github.com/GlobalTechInfo)
